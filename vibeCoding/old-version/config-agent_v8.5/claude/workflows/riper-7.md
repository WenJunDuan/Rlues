...
| **P** Plan+分解 | augment | feature-dev | plan-first + agent-teams | → plan.md + doing.md | ● PLAN_APPROVED |
| **E** Execute | augment + context7 | commit-commands | tdd | ✎ doing.md | ○ BIG_CHANGE |
...
5. Path C+: 用 **explorer** 子代理并行搜索多个方向
...
4. 标注任务依赖关系图:
   - 无依赖 → 标记 `[并行]`
   - 有前置 → 标记 `[串行: 依赖 T{N}]`
5. Path C+: 触发 `feature-dev` plugin 辅助拆解
6. Path D: 读 `skills/agent-teams/SKILL.md` → 标注每任务分配给哪个子代理
   - 按目录/模块划分, 子代理之间零文件重叠
   - explorer 并行调研 → builder 并行实现 → validator 验证
7. 初始化 doing.md (todo list, 所有任务 `- [ ]`)

**写入:**
- `.ai_state/plan.md` — 任务列表 + 依赖图 + 并行标注 + 子代理分配
...
4. 并行调度 (Path C/D):
   ...
   - 分配给 builder 子代理并行执行
   - SubagentStop hook 自动验证每个子代理输出
   - Path D: 创建 Agent Teams, 每 teammate 负责一个模块
5. 每完成一个任务 → 更新 doing.md 进度 (`- [x]`)
6. 每次 commit → `commit-commands` plugin 规范格式
7. PostToolUse hook 自动格式化 + 增量类型检查 (非阻塞)

**写入:** `.ai_state/doing.md` — 实时更新进度
**检查点:** ○ 大规模修改 (>100行或>3文件) → cunzhi `BIG_CHANGE`
**Path D:** 按 `agent-teams` Skill 分配子代理并行
...
**Path C/D:** validator 子代理独立运行测试, 问题记入 review.md 交回 builder
...
2. **Linus 四问** (所有 Path):
   ...
   - 命名清晰? (不需要注释解释)
   - 最简实现? (删到不能再删)
3. **需求回验**: 对照 R₀ 的验收标准逐条检查, 遗漏 → 返回 E
4. **经验沉淀**: 问题 → `pitfalls.md`; 好模式 → `patterns.md`; 工具心得 → `tools.md`
...
需求 → [PACE路由] → Path判定
                    ...
                    Path B+ → R₀a(需求接受) → R₀b(需求讨论 ↺) ──cunzhi──→ R(研究) → D(设计) ──cunzhi──→
                                                                                                        │
  ←── A(验收归档) ←──cunzhi── Rev(审查) ← T(测试) ← E(开发, 并行调度) ←──cunzhi── P(计划+分解+TODO)
```
