# RIPER-7 工作流编排 (Codex CLI)

唯一权威调度表。每个阶段: 加载什么 skill、用什么 MCP、触发什么 Superpowers。
Codex 无官方 Plugins，全部由 Superpowers + MCP + 手动替代。
...
| 阶段 | VibeCoding Skill | MCP | Superpowers Skill | Codex 专属 |
|:---|:---|:---|:---|:---|
| **R** | brainstorm | sou, deepwiki | brainstorming | — |
| **D** | brainstorm | sou, deepwiki, cunzhi | brainstorming | — |
| **P** | — | cunzhi | writing-plans | — |
| **C** | cunzhi | cunzhi | — | — |
| **E** | tdd, worktrees† | sou | subagent-driven-dev, tdd | chrome-devtools† |
| **V** | verification | — | verification-before-completion | chrome-devtools† |
| **Rev** | code-quality, knowledge | cunzhi | requesting-code-review | 手动安全检查 |
...
工具编排:
  ...
  3. → SP brainstorming                ← 苏格拉底式提问 (5-8 问)
...
```
工具编排:
  1. → SP writing-plans                ← 结构化拆解
     读取 ~/.codex/superpowers/skills/writing-plans/SKILL.md
     降级: SP 未安装 → 直接拆解微任务
  2. (无 feature-dev plugin → SP writing-plans 完全覆盖)
  3. Path B/C: TDD 任务交替 (先 RED 后 GREEN)
  4. 每个任务含: 文件路径 + 验证步骤
...
```
工具编排:
  0. Path C: → skill/worktrees 创建隔离分支
  1. → skill/tdd                       ← RED-GREEN-REFACTOR 循环
     内部调用:
       SP tdd                           ← TDD 方法论
       降级: SP 未安装 → 直接 RED-GREEN-REFACTOR
       SP subagent-driven-dev           ← 子任务并行
       降级: SP 未安装 → 顺序执行
  2. sou.search("参考实现")             ← MCP: 搜索相关代码
  3. 前端项目按需:
     → chrome-devtools MCP              ← Codex 专属: 运行时调试
     → skill/context7                   ← 需要查库文档时
  4. 提交策略 (手动 Conventional Commits):
     Path A: 改完直接 commit
     Path B: 每个 TASK 完成后 commit
     Path C: 每个 GREEN 后 commit (小步提交)
...
工具编排:
  1. → skill/verification              ← 验证循环 (按 Path 分级清单)
     内部调用:
       ...
       降级: SP 未安装 → 直接执行清单
  2. 前端: chrome-devtools MCP          ← 验证运行时行为
...
工具编排:
  1. → skill/code-quality              ← 六维审查
     内部调用:
       SP requesting-code-review        ← 替代 code-review + pr-review-toolkit
       降级: SP 未安装 → 直接六维审查
     手动安全检查:
       □ 无硬编码密钥        (替代 security-guidance plugin)
       □ 输入验证
       □ SQL 参数化
       □ 无敏感信息泄露
...
Codex 无 Path D。`--team` 自动降级为 Path C。
