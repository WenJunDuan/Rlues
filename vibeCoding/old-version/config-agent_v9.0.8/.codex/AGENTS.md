# VibeCoding Kernel v9.0.8 — Codex CLI
...
- /plan: 原生规划模式 (P 阶段集成)
- multi_agent + collaboration_modes: 多代理并行 (Path C+, config.toml [features])
- [agents] 角色定义: builder / reviewer / explorer (config.toml [agents.{role}])
- spawn_agents_on_csv: 批量多代理分派
- voice (spacebar): 语音输入 (实验性)
