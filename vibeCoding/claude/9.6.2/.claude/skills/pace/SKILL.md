...
| polish | 主 agent (cleanup, 不 spawn subagent) |
| ship | 主 agent |
...
|---|---|---|
| **plan** (调研) | 1. `context7` (npx ctx7) <br> 2. `augment` MCP (语义检索) <br> 3. WebSearch + WebFetch <br> 4. (option) `antigravity` (agy -p) 外包大型调研 | `~/.claude/skills/context7/SKILL.md` <br> `~/.claude/skills/augment/SKILL.md` <br> `~/.claude/skills/antigravity/SKILL.md` |
| **design** | 同 plan, 加自身推理 | 同上 |
| **impl** | 1. 主 agent (≤ 4 任务) <br> 2. Task `subagent_type: generator` (4-8 任务) <br> 3. `antigravity` agy 并行 (≥ 5 独立任务, 见调度规则) | `~/.claude/skills/antigravity/SKILL.md` (并行 fan-out 场景) |
| **review** | 1. Task `reviewer` + `evaluator` (parallel) <br> 2. `context7` 验证可疑 API 用法 <br> 3. WebFetch 验证 reviewer 提出的疑虑 | `~/.claude/skills/context7/SKILL.md` |
| **polish** | 1. 主 agent 加载 `polish/SKILL.md` <br> 2. (option) `antigravity` 外包大型清理 (≥ 30min) | `~/.claude/skills/polish/SKILL.md` |
