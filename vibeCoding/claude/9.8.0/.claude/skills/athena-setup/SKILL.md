...
# /plugin install code-review@anthropic-official    # 可选: Athena PACE review (reviewer+spec-compliance+evaluator) 已覆盖, 装它仅作 ad-hoc 备选
# /plugin install feature-dev@anthropic-official     # 可选: Athena PACE 路由已覆盖, 同上 (铁律[不重写已有能力]: 不被 Athena skill 调用, 留给用户手动)
