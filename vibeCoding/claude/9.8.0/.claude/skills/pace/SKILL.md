...
# PACE — Router & State Machine (v9.8.0)
...
- plan/design: 第一条 message 加 "ultrathink"; critic 多轮 (max = `_index.plan_critique_max_rounds`)
- impl: 按红黄绿区路由写入; PostToolUse hook 自动写 evidence.yaml
- runtime-verify (Refactor/System 强制 · Feature 可选): /athena-runtime-verify 用 `/goal` 实跑 + 自测自改, 产出 runtime-verify.md (delivery-gate 验)
