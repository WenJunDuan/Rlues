...
# /athena-runtime-verify — 运行时验证环 (v9.8.0)
...
- **实跑** (按类型选已装插件/工具):
  - 前端 / E2E → **playwright-skill** (已装插件: 启 dev server + 浏览器驱动 + 断言)
...
**codex 特例**: System 级密集测试想省 token / 隔离 → 走 athena-route (executor_weight; 本包未含, 待 U5 五五路由落地), opt-in. 不默认。
