...
description: |
  ...
  Athena 在 CC 端可用此 skill 把"重的、可并行的、不需要 CC 自己跑"的任务外包给 Antigravity.
...
- 大批量任务降级到主 agent 顺序处理, 或 Codex `spawn_agents_on_csv` (若 cx 可用)
...
| 2 | Codex `spawn_agents_on_csv` | N ≥ 5 且 `cx_spawn_agents_on_csv = true` |
| 3 | Antigravity `agy -p "..." &` 并行 | N ≥ 5 且 `ag_callable = true` 且无 Codex |
| 4 | 主 agent 顺序处理 | 都不可用 (降级) |
...
- 若 `agy` 命令存在但调用失败 (网络 / 认证): 主 agent log 一条到 `details/runtime-events.md` 标 "ag-call-failed", 自动降级
