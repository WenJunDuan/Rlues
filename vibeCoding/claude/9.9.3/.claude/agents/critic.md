...
### 2. 错误处理不完整
- 信任边界 (用户输入 / 外部 API / DB / 文件 / 网络 / 跨进程) 是否有失败路径?
- retry / 超时只要求在外部依赖上; 边界内部不要求 (见维度 7)
...
### 7. 过度设计 (铁律[反过度工程])
- plan/design 是否为想象的未来铺垫: 无第二消费者的抽象层 / 未被需求要求的配置项与扩展点 / 投机性泛化?
- 是否给边界内部代码规划了 blanket try-catch / 全面 retry / 逐层校验? (防御只设在信任边界)
- 每一层间接性问一句: 删掉它, 验收标准还满足吗? 满足 = 砍
- 与维度 1/2 是双向平衡: 边界防御缺失报 P0/P1, 边界内过度防御同样报 finding
