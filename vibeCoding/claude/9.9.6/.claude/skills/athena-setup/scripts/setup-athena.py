#!/usr/bin/env python3
"""Fresh-install or verify Athena v9.9.6 without overwriting an installation."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fresh-install missing Athena endpoints, or verify v9.9.6."
    )
...
def verify_same(package: Path, kind: str, home: Path) -> bool:
    ...
    try:
        ...
        if kind == "cx":
            ...
            # 9.9.6 installs skills under ~/.agents/skills for native discovery;
            # explicit skills.config registration is stale duplication.
            if entries:
                drifted.append(config)
    except (OSError, ValueError, tomllib.TOMLDecodeError):
        drifted.append(config)
