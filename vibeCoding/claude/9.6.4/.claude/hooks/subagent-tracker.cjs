...
function checkRoadmapAdvance(aiState, idxFm) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes(`sprint_slug: "${sprintSlug}"`) || lines[i].includes(`sprint_slug: ${sprintSlug}`)) {
      foundItemBlock = i;
  ...
  if (foundStatusLine >= 0) {
    lines[foundStatusLine] = lines[foundStatusLine].replace(/status:\s*\w+/, 'status: completed');
    fs.writeFileSync(itemsPath, lines.join('\n'), 'utf-8');

    // 选下个 pending item (拓扑排序简化版)
    const nextPending = findNextPending(lines);
    if (nextPending) {
      updateFrontmatterField(path.join(aiState, '_index.md'), 'next_action', `next_roadmap_item:${nextPending}`);
    } else {
      // 全部 completed, roadmap 结束
      updateFrontmatterField(path.join(aiState, '_index.md'), 'current_roadmap_slug', '');
      updateFrontmatterField(path.join(aiState, '_index.md'), 'next_action', 'roadmap_complete');
    }
  }
}
...
function main() {
  try {
    ...
    const subagentName = payload?.subagent_name || payload?.subagent_type || 'unknown';
    ...
    // 1. 更新 _index.md.last_subagent
    updateFrontmatterField(idxPath, 'last_subagent', subagentName);
    updateFrontmatterField(idxPath, 'last_subagent_at', ts);
    ...
    if (['generator', 'polish_worker'].includes(subagentName) && idxFm.stage === 'ship') {
