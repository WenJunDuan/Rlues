...
cp ~/.claude/skills/pace/templates/roadmap/roadmap.md .ai_state/roadmap/$slug/
cp ~/.claude/skills/pace/templates/roadmap/items.yaml .ai_state/roadmap/$slug/
```

### Step 2: spawn architect 调研 (worktree 隔离)

主 agent 用 Task tool spawn `architect` subagent (read-only, ultrathink):
- 探索代码库
