...
# 4. 写 SSH 别名 (权限面收窄的关键 — settings.json 只放行 athena-vm-* 前缀):
cat >> ~/.ssh/config << EOF
...
- 证据规则不变: **ssh 命令 + 远端输出原样晒进 transcript** (/goal supervisor 只认演示)
...
- ❌ 不存明文密码 (见上, 硬拒绝)
- ❌ 不放行裸 `ssh *` 权限 (只放 `athena-vm-*` 别名前缀, 见 settings.json)
