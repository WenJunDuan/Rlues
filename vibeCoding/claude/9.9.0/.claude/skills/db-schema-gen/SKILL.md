...
1. 读取 Convention Pack，确定数据库方言、迁移目录、命名规则和安全默认项。
2. 产出表设计文档：业务语义、字段、类型、约束、索引、权限归属、数据域、兼容性说明。
3. 产出 DDL SQL：建表、索引、约束、注释、初始化数据或 migration 片段。
4. 互相引用两个产物，保持表名、字段名、索引名逐项一致。
5. 按 Convention Pack 或 `runtime-env` 声明运行 DDL 语法、migration 或本地 DB 验证。
6. 验证失败则修正文档与 SQL，重跑直到通过或标记 blocked。
