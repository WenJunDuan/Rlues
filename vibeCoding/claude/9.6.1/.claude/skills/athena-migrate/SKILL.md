...
# /athena-migrate — v9.5.5 → v9.6 迁移 (v9.6.1)

参考: <https://code.claude.com/docs/en/skills>

> v9.6.1 注: schema_version 仍为 "9.6", patch 级不破坏迁移. 本 skill 仍用于从 v9.5.5 迁移; 已是 v9.6 schema 无需重新迁移.
...
注意: `athena_version` 字段写 "9.6.1" (当前 patch 版本), `schema_version` 写 "9.6" (schema 主版本).
...
```
✓ 迁移完成 (athena_version: 9.6.1, schema: 9.6). 备份在 .ai_state/.legacy-v9.5/.
...
```bash
git add .ai_state/
git commit -m "athena: migrate state to v9.6 schema (v9.6.1)"
```
