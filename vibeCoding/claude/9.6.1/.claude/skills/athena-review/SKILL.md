...
- `executed` — 我跑了命令 / 读了代码 / 验证了输出 (附 tool_use ID 或命令片段)
...
附 `inspected` 或 `assumed` → REWORK (不接受).

### 在 review-report.md 中的写法
...
| 维度 | 分数 | 证据 | 标签 |
|------|------|------|------|
| Functionality | 4 | src/auth/jwt.ts:42-78 实际运行 | executed |
...
外审 (`/codex:review` 或 reviewer subagent) 与内审 (`/review`) 给出**竞争方案**时:
