# /goal Condition 模板库 (跨平台, v9.6.1)

每个 PACE stage 对应一个 condition 模板, 用 `/goal <condition>` 启动 autonomous loop.

参考:
- CC `/goal` (v2.1.139+): <https://code.claude.com/docs/en/goal>
- **Codex `/goal` (v0.128.0+ GA)**: <https://developers.openai.com/codex/changelog>
...
ALL of:
  ...
  - 报告含 test_cmd 实际输出
  - 报告含 executed/inspected/assumed 标签 (v9.6.1 铁律 10)
  - 若有外内审冲突, 报告含 "## Step 7: 矛盾决议" 段 (v9.6.1 铁律 12)
Constraints:
  - 评分必须基于代码行号级证据
  - 跨边界声明仅接受 executed 标签 (v9.6.1 铁律 11)
  - 不超过 40 turns
```
...
### 交互式 (CC v2.1.139+, Codex v0.128.0+)
```
...
Codex: goal 状态跨 session 持久化, 直接 `codex --resume` 续未完成 goal.

### 状态查询
```
/goal             # 不带参数, 看 turns/tokens/最近 evaluator reason
/goal pause       # 暂停 (Codex v0.128+)
/goal resume      # 恢复 (Codex v0.128+)
/goal clear       # 中止
```
...
- condition ≤ 4000 字符
- CC: v2.1.139+ 才有, 旧版本 delivery-gate hook 兜底
- Codex: v0.128.0+ 才有 (rust-v0.128.0 release, 2026-04-30), 旧版本 delivery-gate.py 兜底
