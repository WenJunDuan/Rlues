...
| PreToolUse(Bash) | pre-bash-guard.cjs |
| PostToolUse(Edit/Write/MultiEdit) | index-updater.cjs |
| PostToolUse(Task) | subagent-retry.cjs |
| Stop | delivery-gate.cjs, pace-continuator.cjs |
| PermissionDenied | permission-retry.cjs |
| PreCompact | compact-snapshot.cjs |
| PostCompact | compact-restore.cjs |
| ConfigChange | state-audit.cjs |
...
| 项 | 改动 |
|---|------|
| 铁律 | 9 → 12 (加 校准报告 / 可逆性加权 / 矛盾不折中) |
| META-0 | `<important>` 段末句"第一性分析与铁律冲突 → 命名编号 + 给理由继续" |
| athena-review skill | 加 executed/inspected/assumed 标签规约 + 矛盾决议模板 |
| Codex /goal | 全链对齐 v0.128.0+ GA (cx 端 8 文件) |
