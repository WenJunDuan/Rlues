...
- `cc_version`
- `cx_version`
- `goal_supported` (CC 端): 解析 cc_version, ≥ "2.1.139" 即 true
...
git commit -m "athena: project initialized (v9.6.1 schema)"
```

## 10. 验证

运行 `bash .ai_state/init.sh` 确认脚本正常。
告知用户: "Athena v9.6.1 已就绪。用 `/athena-dev <需求>` 开始开发, 或 `/athena-status` 看面板。"
