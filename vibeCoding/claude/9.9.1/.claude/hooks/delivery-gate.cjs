#!/usr/bin/env node
/**
 * VibeCoding Athena v9.9.1 · CC Stop hook (delivery-gate)
 *
 * 触发: 主 agent stop 前
 * 职责:
 ...
 * 7. v9.9.1: Feature/Refactor/System + ship → subagent-log.md 必须含 generator 的 SubagentStop 记录
...
function main() {
  try {
    ...
    if (['Feature', 'Refactor', 'System'].includes(pathType) && stage === 'ship' && sprintSlug) {
      ...
      if (!fs.existsSync(pass1)) {
        ...
        const bgHint = running.length > 0
          ? `检测到 ${running.length} 个后台任务仍在运行 — 等它们返回后由主 agent 合并产物, 再推进 stage=ship.\n`
          : `reviews/pass1.md 不存在 — 先跑 review 2+1 流程, 由主 agent 合并落盘后再推进 stage=ship.\n`;
        return blockResponse(
      ...
      if (!pass1Content.includes('## Spec Compliance')) {
        return blockResponse(
          ...
          `解锁动作: 运行 spec-compliance, 由主 agent 把返回段落写入 pass1.md 后再 ship.\n`
        );
      }
      ...
      if (REFACTOR_SYSTEM.has(pathType) && !pass1Content.includes('## Evidence Cross-Check')) {
        return blockResponse(
          ...
          `解锁动作: 运行 evaluator, 由主 agent把返回段落写入 pass1.md 后再 ship.\n`
        );
      }
    }
    ...
    if (['Feature', 'Refactor', 'System'].includes(pathType) && stage === 'ship' && sprintSlug && !skipSubagentCheck) {
      ...
      const saContent = fs.existsSync(saLog) ? fs.readFileSync(saLog, 'utf-8') : '';
      const hasGeneratorStop = saContent
        .split(/^##\s+/m)
        .some(section => /·\s*generator\b/i.test(section) && /- Event:\s*SubagentStop\b/i.test(section));
      if (!hasGeneratorStop) {
        return blockResponse(
          `[delivery-gate] 铁律[零写入]: ${pathType} 路径的 impl 必须有 generator 的 SubagentStop 完成记录; 启动记录或名字命中不算完成.\n` +
          `解锁动作: 调用 generator 完成实现并等待其返回; 确属全绿区微改动则设 _index.skip_impl_subagent_check=true (自负责).\n`
        );
      }
    }
