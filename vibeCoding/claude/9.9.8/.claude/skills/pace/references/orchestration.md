# PACE References · Orchestration (v9.9.8)
...
- **产物不豁免**: workflow 跑完仍进一次原生多维 review

**回退链**: workflows 被禁用 (env `CLAUDE_CODE_DISABLE_WORKFLOWS` / Enterprise 默认关) → 退回 Agent subagent 编排, 流程不变.

## /goal · Sisyphus 原生执行层 (M5)

长任务用 `/goal <完成条件>` 设定目标 (CC 2.1.139+), 替代 prompt 层"不完成不许停"约束, 承载铁律[门禁] Sisyphus 语义. ship 前核对 goal 达成.
...
2. stated check (Claude 怎么证明): 如 "打开 reviews/implementation-review.md, 核对 frontmatter `verdict: PASS`"
...
| 不适用 | plan/design 审议留在作者会话; 实现 review 走一次原生多维请求 |
...
- hotfix2 W40 / 9.9.8: assignments 握手只对 writer/generator; 只读 architect/reviewer 直接用 harness identity。CC 2.1.232 起 subagent 默认 fork，2.1.218 默认不嵌套派发，2.1.217 并发默认上限 20。
