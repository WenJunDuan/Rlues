...
6. **读 `skills/brainstorm/SKILL.md` 并严格执行**
7. 逐个提问澄清 → 一次一个, 优先选择题
...
> 读 `skills/plan-first/SKILL.md` — 基于 design.md 生成可执行计划

1. **拆解**: 把 design.md 中的设计转为可执行任务列表
2. **依赖**: 标注任务间依赖关系
3. **并行**: 识别可并行的任务组
4. **估时**: 每个任务预估时间
5. **分配**: Path C 标注子代理分配, Path D 标注 teammate 分配
...
1. **TDD**: 读 `skills/tdd/SKILL.md` — 先写失败测试再实现
2. **实现**: builder 子代理并行开发 (Path C+)
3. **Commit**: 每个子任务完成即 commit (conventional commits)
4. **更新**: 完成的任务从 doing.md 勾选

**技能:** plan-first, tdd
**Path A:** 直接改 + 测试
**Path C+:** builder 子代理并行

## T (测试)

1. **单测**: `npm test` 全部通过
2. **类型**: `tsc --noEmit` 零错误
3. **E2E**: Path C+ 调用 e2e-runner 子代理
4. **手动**: 必要时提示用户手动验证

**技能:** verification, e2e-testing
**子代理:** validator, e2e-runner

## V (验证审查)

1. **Lint**: `npx eslint .`
2. **安全**: security-auditor 子代理扫描 (Path C+)
3. **审查**: validator 子代理综合检查
4. **质量**: 读 `skills/code-quality/SKILL.md` 执行审查清单
...
4. **上下文管理**: 上下文 > 500K 时读 `skills/smart-archive/SKILL.md`
