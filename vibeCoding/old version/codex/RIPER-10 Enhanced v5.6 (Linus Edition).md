...
2. **状态持久化**: 会话结束前必须更新 `/project_document/.ai_state/active_context.md`
...
```
project_document/
├── .ai_state/           # 状态协议存放在项目project_document目录下面
...
### Path A — 闪电 (单文件/<30 行/修复)

| 流程          | 角色 | 协作方式           |
| :------------ | :--- | :----------------- |
| `R1 → E → R2` | LD   | 直接执行，快速验收 |

### Path B — 协作 (2-10 文件/新功能/重构)

| 流程                  | 角色    | 协作方式                     |
| :-------------------- | :------ | :--------------------------- |
| `R1 → I → P → E → R2` | AR + LD | **计划优先**: 敲定方案再执行 |

### Path C — 系统 (>10 文件/架构变更)
