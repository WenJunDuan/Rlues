...
# 9.9.3 baseline = the committed 9.9.2 release trees.  Each endpoint has two
# independent states: working-tree cleanliness and committed tree identity.
CC_992_TREE = "b83120f0b9a25d4f6721a2bffcb04787cbc90a24"
CX_992_TREE = "c8cb51f002ecc9c387fbd8c994ae69e27034c0c1"
CODEX_REQUIRED_VERSION = "0.144.1"
failures: list[str] = []
passes: list[str] = []
skips: list[str] = []
...
def collect_baseline_states(root: Path, expected_trees: dict[str, str]) -> dict[str, bool]:
    states: dict[str, bool] = {}
    endpoints = (
        ("cc", "vibeCoding/claude/9.9.2", "vibeCoding/claude/9.9.2/.claude"),
        ("cx", "vibeCoding/codex/9.9.2", "vibeCoding/codex/9.9.2/.codex"),
    )
    for endpoint, package_path, tree_path in endpoints:
        worktree = subprocess.run(
            ["git", "diff", "--quiet", "HEAD", "--", package_path],
            cwd=root,
            check=False,
            capture_output=True,
            text=True,
        ...
        states[f"{endpoint}_committed"] = (
            committed.returncode == 0 and committed.stdout.strip() == expected_trees[endpoint]
        )
    return states


def check_baseline() -> None:
    states = collect_baseline_states(ROOT, {"cc": CC_992_TREE, "cx": CX_992_TREE})
    for key, label in (
        ("cc_worktree", "CC 9.9.2 baseline unchanged in working tree"),
        ("cx_worktree", "CX 9.9.2 baseline unchanged in working tree"),
        ("cc_committed", "committed CC 9.9.2 tree object unchanged"),
        ("cx_committed", "committed CX 9.9.2 tree object unchanged"),
    ):
        check(label, states[key], key)

    release_roots = (ROOT / "vibeCoding/codex/9.9.3", ROOT / "vibeCoding/claude/9.9.3", ROOT / "vibeCoding/scripts")
    junk = [
    ...
    for release_root in release_roots:
        for path in release_root.rglob("*"):
            ...
            for line in path.read_text(encoding="utf-8").splitlines():
                ...
                # P1-2: the release target itself is 9.9.3, so the ceiling is
                # 9.9.3 — anything beyond it is an unexplained future marker.
                if any((int(major), int(minor), int(patch)) > (9, 9, 3) for major, minor, patch in versions):
...
def check_identity_and_config() -> None:
    ...
    check("config version=9.9.3", version == "9.9.3", repr(version))
    ...
    check("AGENTS identity=9.9.3", "v9.9.3" in text(CX / "AGENTS.md"))
    check("CLAUDE identity=9.9.3", "v9.9.3" in text(CC / "CLAUDE.md"))
    for endpoint, root in (("CX", CX), ("CC", CC)):
        ...
        check(f"{endpoint} active skill titles identify 9.9.3", not stale_titles, repr(stale_titles))
...
def check_contract_text() -> None:
    ...
    for endpoint, root in (("CX", CX), ("CC", CC)):
        ...
        check(f"{endpoint} init active identity=9.9.3", "v9.9.3" in init_body and "初始化完成" in init_body)
    ...
    for endpoint, release_root in (("CX", CX.parent), ("CC", CC.parent)):
        ...
        check(f"{endpoint} RELEASE uses current runtime commands", "test-athena-claude-9.9.3-runtime.cjs" in release and "test-athena-9.9.3-runtime.py" in release)
        check(f"{endpoint} RELEASE has no wrong Node Python command", "node vibeCoding/scripts/test-athena-9.9.3-runtime.py" not in release)
        check(f"{endpoint} RELEASE records CC runtime 0-FAIL baseline", bool(re.search(r"\d+ PASS / 0 FAIL", release)))
...
def check_install_contract() -> None:
    ...
    check("setup finds repo-root CX package", "vibeCoding/codex/9.9.3/.codex" in setup)
    check("setup finds repo-root CC package", "vibeCoding/claude/9.9.3/.claude" in setup)
    for state in ("fresh", "CC-only", "CX-only", "same-version", "old-version"):
        check(f"setup documents {state}", state in setup)
    # 9.9.3 (design §7.2, user decision): migration is AI-guided; scripted
    ...
    check("migrate is AI-guided to 9.9.3", "AI" in migrate and "9.9.3" in migrate)
    ...
    for marker in ("备份", "preserve", "rollback", "密钥", "quantum-codegen", "quantum-data", "project-data-reader", "9.9.3"):
...
class JsonSubprocessResult(NamedTuple):
    ok: bool
    payload: Any | None
...
def diagnostic_excerpt(value: str | bytes | None, limit: int = 600) -> str:
    if value is None:
        return ""
...
def run_json_subprocess(
    ...
    *,
    ...
    try:
        ...
        detail = (
            f"timeout command={command_label!r} stdout={diagnostic_excerpt(exc.stdout)!r} "
            f"stderr={diagnostic_excerpt(exc.stderr)!r}"
        )
        return JsonSubprocessResult(False, None, "timeout", detail)
    except OSError as exc:
        return JsonSubprocessResult(
            False,
            None,
            "spawn_error",
            f"spawn_error command={command_label!r} error={type(exc).__name__}: {exc}",
        )
    if run.returncode != 0:
        try:
        ...
        return JsonSubprocessResult(
            ...
            payload,
            "nonzero_exit",
            f"nonzero_exit command={command_label!r} exit={run.returncode} "
            f"stdout={diagnostic_excerpt(run.stdout)!r} stderr={diagnostic_excerpt(run.stderr)!r}",
        )
    try:
        payload = json.loads(run.stdout)
    except json.JSONDecodeError as exc:
        return JsonSubprocessResult(
            ...
            None,
            "invalid_json",
            f"invalid_json command={command_label!r} error={exc.msg} "
            f"stdout={diagnostic_excerpt(run.stdout)!r} stderr={diagnostic_excerpt(run.stderr)!r}",
        )
    return JsonSubprocessResult(True, payload, None, f"ok command={command_label!r}")


def codex_version_matches(output: str) -> bool:
    return re.search(rf"(?<![\d.]){re.escape(CODEX_REQUIRED_VERSION)}(?![\d.])", output) is not None


def find_exact_codex_binary() -> tuple[str | None, str]:
    candidates = [
        os.environ.get("ATHENA_CODEX_BIN", ""),
    ...
    for candidate in candidates:
        ...
        try:
            ...
            diagnostics.append(f"{candidate}: timeout")
            continue
        except OSError as exc:
            diagnostics.append(f"{candidate}: spawn_error {type(exc).__name__}: {exc}")
            continue
        output = " ".join((run.stdout + " " + run.stderr).split())
        diagnostics.append(f"{candidate}: exit={run.returncode} output={output[-240:]!r}")
        if run.returncode == 0 and codex_version_matches(output):
            return candidate, diagnostics[-1]
    return None, "; ".join(diagnostics) or "no Codex candidates"


def check_fresh_codex_runtime() -> None:
    setup_script = CX / "skills/athena-setup/scripts/setup-athena.py"
    codex, version_detail = find_exact_codex_binary()
    if codex is None:
        check(f"Codex runtime exact {CODEX_REQUIRED_VERSION}", False, version_detail)
        check("temp HOME Codex doctor", False, "exact Codex runtime unavailable")
        check("temp HOME prompt-input", False, "exact Codex runtime unavailable")
        return
    check(f"Codex runtime exact {CODEX_REQUIRED_VERSION}", True, version_detail)
    ...
    with tempfile.TemporaryDirectory(
        ...
        doctor = run_json_subprocess(
            [codex, "--strict-config", "doctor", "--json"],
            cwd=ROOT,
            env=env,
            timeout=90,
        )
        if isinstance(doctor.payload, dict):
            ...
            detail = f"exit={'0' if doctor.ok else 'nonzero'} config={config_status} provider={provider_status} auth={auth_status} (auth fail accepted)"
            ...
            check("temp HOME Codex doctor redacted JSON", False, doctor.detail)

        prompt = run_json_subprocess(
        ...
        if prompt.ok:
            ...
            identity = "VibeCoding Athena v9.9.3" in rendered
            ...
            check(
                ...
                identity and pace and agents_skills,
                f"identity={identity} pace={pace} agents_skills={agents_skills}",
            )
        else:
            check("temp HOME prompt-input Athena/PACE/.agents", False, prompt.detail)
...
def check_runtime_contract() -> None:
    runtime_suite = ROOT / "vibeCoding/scripts/test-athena-9.9.3-runtime.py"
    ...
    cc_runtime_suite = ROOT / "vibeCoding/scripts/test-athena-claude-9.9.3-runtime.cjs"
    ...
    if node is None:
        ...
        cc_run = subprocess.run(
            ...
            capture_output=True,
            text=True,
            env={**env, "ATHENA_SKIP_LIVE_CLAUDE_MATRIX": "1"},
        )
