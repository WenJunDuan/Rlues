#!/usr/bin/env python3
"""Runtime behavior contract for the Athena v9.9.3 Codex package.
...
HOOKS = ROOT / "vibeCoding/codex/9.9.3/.codex/hooks"
FIXTURES = ROOT / "vibeCoding/scripts/fixtures/athena-9.9.3"
...
USER_PROMPT_SUBMIT = HOOKS / "user-prompt-submit.py"
CX_EVALUATOR = ROOT / "vibeCoding/codex/9.9.3/.codex/agents/evaluator.toml"
CX_SETUP = ROOT / "vibeCoding/codex/9.9.3/.codex/skills/athena-setup/scripts/setup-athena.py"
VALIDATOR = ROOT / "vibeCoding/scripts/validate-athena-9.9.3.py"
...
def bind_review(project: Path, sprint_dir: Path, review_path: Path) -> None:
    ...
    manifest_paths = {
        ...
        "architecture/athena-9.9.3.md": project / ".ai_state/architecture/athena-9.9.3.md",
    }
    manifest_lines = [
        "schema_version: 1",
...
def build_complete_feature(project: Path) -> Path:
    ...
    (architecture / "athena-9.9.3.md").write_text("# Athena 9.9.3\n", encoding="utf-8")
    ...
    (sprint_dir / "tdd-evidence.yaml").write_text(
        "schema_version: 1\nrecords:\n"
        "  - test_file: vibeCoding/scripts/test-athena-9.9.3-runtime.py\n"
        "    red_command: python3 vibeCoding/scripts/test-athena-9.9.3-runtime.py\n"
        ...
        "    green_command: python3 vibeCoding/scripts/test-athena-9.9.3-runtime.py\n"
...
class BreadcrumbContractTests(unittest.TestCase):
    def write_stage_source(self, home: Path, relative: str, marker: str) -> None:
        stages = home / relative / "pace/references/stages.md"
        stages.parent.mkdir(parents=True, exist_ok=True)
        stages.write_text(
            "## ship\n" + "\n".join(f"- {marker} obligation {index}" for index in range(1, 15)) + "\n",
            encoding="utf-8",
        )

    def run_breadcrumb(self, script: Path, project: Path, home: Path) -> subprocess.CompletedProcess[str]:
        env = os.environ.copy()
    ...
    def test_canonical_agents_skill_wins_over_stale_codex_copy_and_total_is_ten_lines(self) -> None:
        with tempfile.TemporaryDirectory(prefix="athena-993-breadcrumb-") as raw_dir:
            root = Path(raw_dir)
            home = root / "home"
            project = root / "project"
            write_index(project, stage="ship")
            self.write_stage_source(home, ".agents/skills", "canonical")
            self.write_stage_source(home, ".codex/skills", "stale")

            result = self.run_breadcrumb(USER_PROMPT_SUBMIT, project, home)

            self.assertEqual(result.returncode, 0, result.stderr)
            payload = json.loads(result.stdout)
            context = payload["hookSpecificOutput"]["additionalContext"]
            self.assertIn("canonical obligation", context)
            self.assertNotIn("stale obligation", context)
            self.assertIn("~/.agents/skills/pace/references/stages.md", context)
            self.assertLessEqual(len(context.splitlines()), 10, context)

    def test_fresh_setup_installs_the_source_consumed_by_breadcrumb(self) -> None:
        with tempfile.TemporaryDirectory(prefix="athena-993-breadcrumb-setup-") as raw_dir:
            root = Path(raw_dir)
            home = root / "home"
            project = root / "project"
            write_index(project, stage="ship")
            setup = subprocess.run(
                [sys.executable, str(CX_SETUP), "--home", str(home), "--repo-root", str(ROOT), "--only", "cx"],
                cwd=ROOT,
                capture_output=True,
                text=True,
                check=False,
            ...
            installed_hook = home / ".codex/hooks/user-prompt-submit.py"
            result = self.run_breadcrumb(installed_hook, project, home)
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertTrue(result.stdout.strip(), "fresh CX setup produced no breadcrumb")
            context = json.loads(result.stdout)["hookSpecificOutput"]["additionalContext"]
            self.assertLessEqual(len(context.splitlines()), 10, context)


class EvaluatorPolicyContractTests(unittest.TestCase):
    def test_cx_unresolved_over_engineering_matrix_caps_concerns(self) -> None:
        body = CX_EVALUATOR.read_text(encoding="utf-8")
        self.assertRegex(body, r"unresolved_over_engineering\s*(?:>=|≥)\s*1")
        self.assertRegex(body, r"(?:1\s*(?:or|/|或|、|-|–)\s*2|exactly 1 or 2).{0,160}CONCERNS")
        self.assertRegex(body, r"resolved over-engineering.{0,200}(?:does not|不).{0,80}(?:cap|限制|触发)")


def load_validator_module():
    spec = importlib.util.spec_from_file_location("athena_993_validator", VALIDATOR)
...
class BaselineIntegrityContractTests(unittest.TestCase):
    def setUp(self) -> None:
        self.validator = load_validator_module()

    def make_repo(self) -> tuple[Path, tempfile.TemporaryDirectory[str], dict[str, str]]:
        temporary = tempfile.TemporaryDirectory(prefix="athena-993-baseline-")
        repo = Path(temporary.name)
        for relative in (
            "vibeCoding/claude/9.9.2/.claude",
            "vibeCoding/codex/9.9.2/.codex",
        ):
            target = repo / relative / "marker.txt"
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_text(relative + "\n", encoding="utf-8")
        git(repo, "init", "-q")
        git(repo, "config", "user.email", "athena@example.invalid")
        git(repo, "config", "user.name", "Athena Contract")
        git(repo, "add", ".")
        git(repo, "commit", "-qm", "baseline")
        expected = {
            "cc": git(repo, "rev-parse", "HEAD:vibeCoding/claude/9.9.2/.claude"),
            "cx": git(repo, "rev-parse", "HEAD:vibeCoding/codex/9.9.2/.codex"),
        }
        return repo, temporary, expected

    def test_clean_baseline_has_all_four_green_states(self) -> None:
        repo, temporary, expected = self.make_repo()
        try:
            states = self.validator.collect_baseline_states(repo, expected)
    ...
    def test_each_baseline_state_fails_independently(self) -> None:
        cases = (
            ("cc_worktree", "vibeCoding/claude/9.9.2/.claude/marker.txt", None),
            ("cx_worktree", "vibeCoding/codex/9.9.2/.codex/marker.txt", None),
            ("cc_committed", None, "cc"),
            ("cx_committed", None, "cx"),
        )
        for failed_key, dirty_path, bad_hash_key in cases:
            with self.subTest(failed_key=failed_key):
                ...
                try:
                    ...
                    if bad_hash_key:
                        expected[bad_hash_key] = "0" * 40
                    states = self.validator.collect_baseline_states(repo, expected)
                    self.assertFalse(states[failed_key], states)
                    self.assertTrue(all(value for key, value in states.items() if key != failed_key), states)
                finally:
                    temporary.cleanup()


class SubprocessDiagnosticContractTests(unittest.TestCase):
    def setUp(self) -> None:
        self.validator = load_validator_module()

    def test_json_subprocess_reports_all_fail_closed_failure_kinds(self) -> None:
        python = sys.executable
        cases = (
            ("spawn_error", ["/definitely/missing/athena-command"]),
            ("timeout", [python, "-c", "import time; time.sleep(1)"]),
            ("nonzero_exit", [python, "-c", "import sys; print('boom', file=sys.stderr); sys.exit(7)"]),
            ("invalid_json", [python, "-c", "print('not-json')"]),
        )
        for expected_kind, command in cases:
            with self.subTest(expected_kind=expected_kind):
                result = self.validator.run_json_subprocess(command, cwd=ROOT, env=os.environ.copy(), timeout=0.05)
                self.assertFalse(result.ok)
                self.assertEqual(result.failure_kind, expected_kind)
                self.assertIn(expected_kind, result.detail)
        success = self.validator.run_json_subprocess(
            [python, "-c", "print('{\\\"ok\\\": true}')"],
            cwd=ROOT,
            env=os.environ.copy(),
            timeout=5,
        )
        self.assertTrue(success.ok, success.detail)
        self.assertEqual(success.payload, {"ok": True})

    def test_codex_runtime_requires_exact_0_144_1_version_evidence(self) -> None:
        self.assertTrue(self.validator.codex_version_matches("codex-cli 0.144.1"))
        self.assertFalse(self.validator.codex_version_matches("codex-cli 0.144.0"))
        self.assertFalse(self.validator.codex_version_matches("codex-cli 0.145.0"))
