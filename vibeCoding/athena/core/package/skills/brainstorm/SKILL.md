...
| 语义模糊: 主 agent 无法从输入直接写出可观测验收标准 （宪法「路由」） | 自动 | ✅ |
...
3. **能自己查的不问用户**: 答案在代码 / .ai_state / architecture / decisions 里 → Grep/Read 自行确认, 连续解决掉的问题只汇报一句
4. **顶住模糊回答**: "以后再说" / "大概X吧" / "都行" = 继续钻的信号, 不是过关信号; 允许指出矛盾与含糊, 礼貌但不接受雾
5. **半答案给 strawman**: 用户说 "不知道, 可能X" → 给一个可反对的完整草案 "这样定, 哪里不对?" — 反对比发明容易
6. **足以形成可观察验收就结束** — 未消除的关键歧义才继续，不设固定追加问题数量
7. **用可审查合同收口**：总结已确认目标、约束和可观察验收，进入 plan 或按独立切片进入 roadmap。

## 例外

- 用户明确说 "去查 X 的资料" → 主 agent 可调度 只读子 agent / WebSearch (显式调研)
- 用户说 "看下我们之前怎么决定 Y" → 主 agent 可读 decisions/*.md
