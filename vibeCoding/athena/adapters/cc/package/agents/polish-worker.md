...
- 只做与当前合同相关的清理，五项各一行结论：死代码与未用导出；重复与过度抽象（~/.claude/rules/coding.md 反过度工程）；命名与注释（~/.claude/rules/docs.md）；错误处理一致性；安全扫描（~/.claude/rules/security.md：日志里的密钥、未锁依赖）。
