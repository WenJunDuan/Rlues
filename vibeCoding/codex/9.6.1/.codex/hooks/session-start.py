#!/usr/bin/env python3
"""VibeCoding Athena Codex SessionStart hook v9.6.1.
...
v9.6.1 变更: 删除 active_goal 注入时的 "(CC only)" 标签 (Codex /goal v0.128.0+ 已 GA).
"""
import json
import os
import re
...
def main():
    ...
    for p in (PROJECT_ESSENTIALS, GLOBAL_ESSENTIALS):
        ...
        if text:
            ...
            break
    if not lines:
        lines.append("[VibeCoding Athena Codex v9.6.1] TDD · Review · Sisyphus · 索引先行")
        ...
        try:
            ...
            if fm:
                ...
                if goal:
                    ...
                    # v9.6.1: 移除 "(CC only)", Codex /goal v0.128+ 已 GA
                    lines.append(f"[/goal active] {snippet}")
