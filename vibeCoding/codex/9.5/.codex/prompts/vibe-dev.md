...
.ai_state/ 不存在 → 提示 `/vibe-init` 并停。

## Get-bearings (R₀, v9.5 升级)

1. **全局**: 扫 `~/.codex/lessons/INDEX.md` → 找命中本任务主题, 读对应 lesson 文件
...
9. **4 元素口诀自检** (AGENTS.md 顶部): 上下文 + 目标 + 约束 + 验证 都齐了吗?

## Dispatch

有进行中的 stage → 从当前阶段继续 (通过 $pace 触发 pace skill)。

新需求 → 通过 $pace 触发 pace skill 路由, 需求内容: $ARGUMENTS
...
| scenario | 模板 | 重点 |
|----------|------|------|
| `from-zero` | `~/.codex/skills/pace/prompts/from-zero.md` | 图 04: Idea→Spec→Architecture→Tasks→Code |
| `modify-existing` | `~/.codex/skills/pace/prompts/change-existing.md` | 图 06: Read→Locate→Plan→Patch→Verify, 强制 change-plan.md |
