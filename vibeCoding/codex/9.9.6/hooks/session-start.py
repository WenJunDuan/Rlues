...
def special_alerts(fm: dict, ai_state: Path | None = None) -> list:
    alerts = []
    ...
    # 未消解的 delivery-gate 熔断升级 (design §10.1 AC16i)。熔断"不是放水"的承重腿:
    # escalation 不发 block, 若无人告警就等于静默吞掉失败 —— 尤其外包 exec 会话的
    # stderr 不回喂编排者, 编排侧只会看到一次干净的 Stop。
    escalation = pending_escalation(ai_state, fm.get("current_sprint_slug", ""))
    if escalation:
        slug = fm.get("current_sprint_slug", "")
        alerts.append(
            f"🛑 **门禁升级未消解**: 上次 Stop 连续 {escalation.get('consecutive')} 次同因阻断后已熔断 "
            f"(ESCALATED, {escalation.get('ts')})。阻断本身**未解除**, 只是停止了空转。"
            f"查 .ai_state/sprints/{slug}/stop-failures.jsonl, 修掉根因再继续; 不得当作已通过。"
        )

    return alerts


def pending_escalation(ai_state, sprint_slug: str):
    """尾部是 GateEscalated 且其后无 GatePass = 升级尚未消解。失败即静默 (fail-open)。"""
    if ai_state is None or not sprint_slug:
        return None
    try:
        raw = (Path(ai_state) / "sprints" / sprint_slug / "stop-failures.jsonl").read_text(encoding="utf-8")
    except OSError:
        return None
    last = None
...
def main() -> int:
    try:
        ...
        if ai_state:
            ...
            alerts = special_alerts(fm, ai_state)
            if alerts:
