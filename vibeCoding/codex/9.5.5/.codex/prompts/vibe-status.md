# /vibe-status — VibeCoding 状态面板 (Codex 端 v9.5)
...
- 当前 model + profile
- 已装插件 `/plugins list`（Codex 3/27/2026 上线插件市场）
- `npx ctx7 --version 2>&1 | head -1`

## 输出格式

```
=== VibeCoding Hermes v9.5 (Codex) Status ===
...
[Lessons] 项目: 4 条 (最新: "JWT refresh secret 分离")
          [全局已删除, 跨项目记忆装 superpowers]

[Codex]   version       ✓ codex-cli 0.124.0
          codex_hooks   ✓ stable
          model         gpt-5.5 (default)
          plugins       4 装机 (superpowers, slack, linear, ...)
