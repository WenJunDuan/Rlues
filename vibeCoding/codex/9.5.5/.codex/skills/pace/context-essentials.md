...
4. Review 强制 (Feature+ 至少一次交叉审查: 主力+/review+reviewer)
...
- impl 每完成 Task → tasks.md 勾选 + progress.md 追加一行
- 发现 design.md 错 → 先改 design.md 再继续写代码
- V 阶段 Gate 通过 → $compound 追加 .ai_state/lessons.md (项目级)

## 6 路径

Hotfix(无仪式) · Bugfix(定位+TDD) · Quick(TDD+收尾) · Feature(顺序+增强) · Refactor(spawn_agent 并行+增强) · System(全链)
...
- **查阅**: context7 (ctx7 CLI)
- **守卫**: hooks (delivery-gate / bash-guard / session-start / user-prompt-submit / subagent-retry)
