...
3. spawn_agent `~/.codex/agents/reviewer.toml` → 写 findings 到 `details/reviews/sprint-{N}.md`
4. spawn_agent `~/.codex/agents/evaluator.toml` → 追加 VERDICT
5. 根据 VERDICT 决定下一步:
   - PASS / CONCERNS (Refactor/System) → 提示进 polish stage
   - PASS / CONCERNS (其他路径) → 提示进 ship
   - REWORK / FAIL → 提示回 impl

详见 `~/.codex/agents/reviewer.md` 和 `~/.codex/agents/evaluator.md`.
