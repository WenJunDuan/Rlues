#!/usr/bin/env python3
"""
VibeCoding Athena v9.6.4 · Codex SessionStart hook

触发: session 启动 / resume
职责:
1. 注入 _index.md frontmatter 摘要
2. 注入 ~/.agents/standards/_index.md 摘要
3. v9.6.4 新: stage-specific 操作提示 (xhigh / critic / spec-compliance)
4. v9.6.4 新: design_changed_after_impl=true 强提示
5. v9.6.4 新: next_action = roadmap 自动推进提示
...
def stage_hints(fm: dict) -> list[str]:
    stage = fm.get("stage", "")
    hints = []
    ...
    if stage == "impl":
        hints.append("🔧 **impl stage**: 写代码必须用 spawn_agent (铁律 12).")
        path_type = fm.get("path", "")
        if path_type in ("Refactor", "System"):
            hints.append(f"⚠️ path={path_type}: 主 thread 先 git worktree add 后 spawn_agent --cwd <wt-path>.")
...
def special_alerts(fm: dict) -> list[str]:
    alerts = []
