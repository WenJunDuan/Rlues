...
name: athena-review
description: |
  PACE review stage 执行 skill (Codex). 6 维度 (并行 spawn 3 subagent: reviewer + spec-compliance + evaluator).
  v9.7.0: 由 deprecated prompts/ 迁为 skill; 清除 Stop hook prompt 类型残留引用 (该类型已删, 门禁统一由 delivery-gate 在 ship stage 执行).
effort: high
---

# /athena-review — Review stage (Codex, v9.7.0)
...
```
# 主 thread 并行调用 (multi-agent v2)
spawn_agent ~/.codex/agents/reviewer.toml         # 1-5 维度
spawn_agent ~/.codex/agents/spec-compliance.toml  # 6 维度
wait
# 等两个完成后, spawn evaluator (后跑, 综合两家)
spawn_agent ~/.codex/agents/evaluator.toml
```
...
shell `git diff main...HEAD --stat + --name-only`.
...
完整性由 `delivery-gate.py` hook (Stop) 在 **ship** stage 强制:
- pass1.md 不存在 → block
...
- ❌ 用同一个 reviewer 跑多次 (要换 agent toml, 借 OMO Metis 多 critic 思想)
