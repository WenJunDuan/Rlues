...
# PACE — Router & State Machine (v9.9.8, Codex)
...
- plan/design: 作者写 design + 派生 packet；不 spawn critic。R/S 或用户显式要求才由非作者会话按 packet 挑战。
- **spec-gate impl-entry**: Feature+ 先验验收标准 + packet hash/AC 双射（≤80 行）。
- impl: 按红黄绿区写入
- runtime-verify (R/S 强制): `/goal` 实跑
- polish (R/S): 会改代码的清理在 review 之前
- review: 一次原生多维请求（异步）；`await-review-result` 时 Stop 放行
