#!/usr/bin/env python3
"""
VibeCoding Athena v9.8.0 · Codex PostToolUse(Bash) hook
...
职责 (兼容旧 spawn_agent Bash 调用日志; 原生 SubagentStop 由 subagent-tracker.py 处理):
  ...
  5. v9.8.0 Loop Engineering: generator 完成且 stage=impl 且 checklist 全完成 → 写 next_action (System/Refactor=runtime-verify, 其余=review)
...
def impl_tasks_complete(ai_state: Path, sprint_slug: str) -> bool:
    ...
    if not cl.exists():
        return True  # 无 checklist (绿区小改) → 视为完成
...
def main() -> int:
    try:
        ...
        if is_subagent_cmd:
            ...
            # v9.8.0 Loop Engineering: generator 完成且 stage=impl 且 checklist 全完成 → 写 next_action
            if (exit_code == 0 and stage == "impl" and agent_name == "generator"
                    and impl_tasks_complete(ai_state, sprint_slug)):
                idx_p = ai_state / "_index.md"
                rt_skip = read_field(idx_p, "skip_runtime_verify").lower() == "true"
                path_t = read_field(idx_p, "path")
                next_act = "runtime-verify" if (not rt_skip and path_t in ("Refactor", "System")) else "review"
                update_field(idx_p, "next_action", next_act)
