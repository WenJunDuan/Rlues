#!/usr/bin/env python3
"""
VibeCoding Athena v9.7.0 · Codex SubagentStart/SubagentStop hook

职责:
  1. SubagentStart/SubagentStop 触发时写 sprints/{slug}/subagent-log.md (生命周期日志, 按事件标注)
  2. 更新 _index.md last_subagent + last_subagent_at

v9.9.0: hooks.json 同时注册 SubagentStart 与 SubagentStop → 用官方公共字段
hook_event_name 区分事件. next_action 推进只在 SubagentStop 执行 (Start 无完成语义).
payload 缺 hook_event_name 时按 SubagentStop 兼容 (旧版 Codex 无 SubagentStart 事件).
...
def main() -> int:
    try:
        ...
        event = event.strip() if isinstance(event, str) and event.strip() else "SubagentStop"
        ...
        # v9.9.0: 仅 SubagentStop 执行 — SubagentStart 只记生命周期, 不做 roadmap 推进.
        exit_code = payload.get("exit_code", payload.get("exit", 0))
        stage = read_field(idx_path, "stage")
        if (event == "SubagentStop"
                and sprint_slug and agent_name == "generator" and stage == "impl"
