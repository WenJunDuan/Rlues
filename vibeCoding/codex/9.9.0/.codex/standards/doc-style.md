...
## 电报体 (v9.9.0 · agent 输出纪律)

- 骨架句: 名词+动词+数字。删: 铺垫/客套/过渡句/复述输入/总结已写内容
- 表格 > 列表 > 散文; 引文件给路径, 不贴原文
- 依据 (o200k 实测): 电报体较白话省 21-30% token; 文言文字符 -35% 但生僻字 token 单价 0.7→1.0/字符, 净省 ≈0 且规范文档歧义不可接受 → 否决
- 边界: thinking/审议推理不受限 (质量>省字); delivery-gate block reason 与解锁动作必须完整; 面向人的正式文档 (README/API 说明) 用完整句
</important>
