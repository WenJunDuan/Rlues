...
description: |
  ...
  v9.9.0: 路由审议思维链 — 感知→假设→四维权衡→决策+置信度→落盘 route-note; 中途 re-route 只升不降.
effort: medium
---

# /athena-dev — Athena 主入口 (Codex, v9.9.0)

## 触发

用户进任意项目, 说 "开始", "做个 X", "帮我 Y" 等. 主 agent 进入路由分诊.

## 路由审议 (铁律[分诊] · v9.9.0 思维链协议)

**前置**: 无 `.ai_state/` → 提示先跑 /athena-init, 停. 用户显式声明生产事故/hotfix → 直接进 plan(Hotfix) (唯一免审议).

路由是 triage, 不是查表. 每次收到任务, 在回复里走一遍审议 (对用户可见, 3-8 行, 不长篇大论):

### Step 0 · 感知 (30 秒内能拿到的事实)
...
### Step 1 · 假设
提出 ≥2 个候选 (路径或 stage), 各列支持/反对证据. 只有一个候选 = 没认真想.
...
### Step 5 · 落盘
`sprints/{slug}/route-note.md` (≤10 行, 模板见 pace/templates/sprints/route-note.md) + 更新 `_index.route_confidence`.
...
cp ~/.codex/skills/pace/templates/sprints/{design.md,checklist.yaml,route-note.md} ".ai_state/sprints/${slug}/"

update_field path "${path_type}"
update_field route_confidence "${confidence}"   # v9.9.0 审议 Step 3 产出
