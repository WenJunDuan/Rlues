...
# 4. 写 SSH 别名 (权限面收窄的关键 — CC 端 settings.json 只放行 athena-vm-* 前缀;
#    CX 端等价: 命令权限面同样按 athena-vm-* 别名前缀收窄, 见 config.toml approval/sandbox 配置):
cat >> ~/.ssh/config << EOF
...
- ❌ 不存明文密码 (见上, 硬拒绝)
- ❌ 不放行裸 `ssh *` 权限 (只放 `athena-vm-*` 别名前缀; CC 端见 settings.json, CX 端在 config.toml approval/sandbox 层收窄)
