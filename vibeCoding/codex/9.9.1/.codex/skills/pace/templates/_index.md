...
# v9.9.1 schema. 项目执行 athena-init 时由模板初始化, 之后由主 agent + hooks 维护.
version: "9.9.1"
...
platform_features:
  cc_subagent_task: true          # CC subagent capability
...
# Athena Project State Index (v9.9.1)
...
- `spawn_agent` 启动 critic (独立 context, read-only); 主 agent 将返回段落写入 design.md
...
- Refactor/System: CC 用当前 isolation 能力; CX 由主 thread `git worktree add`, 任务携带绝对路径, agent 用 `pwd`/`workdir` 验证
