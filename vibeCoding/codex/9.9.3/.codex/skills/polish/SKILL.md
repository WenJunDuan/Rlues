...
# /polish — Polish stage (v9.9.3, Codex)
...
### Step 1: 调用 polish worker

主 agent 用 `spawn_agent` 创建 `polish_worker` 任务. `message` 写明当前 sprint、worktree 绝对路径、允许写集、5 个检查项、review findings 与验证命令. Agent 首个命令运行 `pwd`, 后续 shell 显式设置 worktree `workdir`.

polish worker 修改代码并返回清理摘要; `.ai_state` 产物由主 agent 根据实际 diff 与返回结果复核后落盘.
