...
ls ~/.codex/hooks/*.py | wc -l   # 应为 10:
# session-start / user-prompt-submit / index-updater / pre-bash-guard / subagent-retry
# / delivery-gate / evidence-collector / compact-snapshot / compact-restore / subagent-tracker
...
```bash
# 7 个 athena-* entry skill 以 skill 形态部署, 文件名一律 SKILL.md (config.toml [[skills.config]] 指向 SKILL.md).
...
# Rules (6 个 md: _index + coding/doc/git/security/ui)
...
ls ~/.claude/hooks/*.cjs | wc -l    # 应为 14
# Agents (5 个 .md: critic / evaluator / generator / reviewer / spec-compliance)
mkdir -p ~/.claude/agents && cp cc/.claude/agents/*.md ~/.claude/agents/
ls ~/.claude/agents/*.md | wc -l    # 应为 5
# Skills
