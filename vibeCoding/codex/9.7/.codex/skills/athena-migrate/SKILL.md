...
name: athena-migrate
description: |
  Athena 版本迁移工具. 含 v9.6.2 → v9.6.4 破坏式重构 (sprints/ + compound/ + lessons.md 三选项交互).
  v9.7.0: 由 deprecated prompts/ 迁为 skill; 修 Step 7 跨端命令残留 (node ~/.claude/… → python3 ~/.codex/hooks/…).
effort: medium
---

# /athena-migrate — Athena 版本迁移 (Codex, v9.7.0)
...
| **v9.6.4 → v9.7.0** | 低 | 非破坏 (覆盖配置文件 + 删 prompts/, .ai_state 数据结构不变) |
...
```bash
# 跑 index-updater 重新扫一遍 (Codex 端是 Python hook)
python3 ~/.codex/hooks/index-updater.py
...
- ⚠️ **破坏式迁移仅运行一次**, 完成后再次运行会跳过 (检测 `_index.md.version` 字段)
- ⚠️ 主 agent 必须用 xhigh 处理破坏式迁移 (复杂度高)
