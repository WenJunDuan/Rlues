...
# PACE — Router & State Machine (v9.9.1, Codex)
...
2. **比较候选**: 至少 2 个候选路径, 记录各自支持/反对证据
3. **权衡**: 爆炸半径 × 可逆性 × 紧急度 × 需求不确定性
4. **决策 + 置信度**: ≥0.8 直接进; 0.5–0.8 记录假设与廉价退出点; <0.5 问 1-2 个决定性问题或进 brainstorm
...
- plan/design: `plan_mode_reasoning_effort = xhigh` 已生效; critic 多轮 (max = `_index.plan_critique_max_rounds`); `_index.plan_model: fable` 为 CC 端专属 (claude-fable-5), CX 端等价姿势 = xhigh reasoning effort (已是默认)
- impl: 按红黄绿区路由写入; hook 记录能观察到的过程证据, 文件证据由 `git diff` 现场核验
- runtime-verify (Refactor/System 强制 · Feature 可选): 实跑 + 自测自改, 产出 runtime-verify.md; 仅在用户显式要求或已有 Goal 时用 Goals 承载
- review: 并行 reviewer + spec-compliance 返回结果; 主 thread 合并 passN.md 后再跑 evaluator, 最后由主 thread 更新 `_index.next_action`
