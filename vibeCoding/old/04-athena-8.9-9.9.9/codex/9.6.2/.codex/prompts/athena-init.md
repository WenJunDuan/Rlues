...
CC_VERSION=$(codex --version 2>/dev/null || echo "claude-code (unknown)")
...
# augment MCP (CC 端在 ~/.codex/config.toml [mcp_servers] 中)
...
mkdir -p .ai_state/details/lessons
```
...
- `tools_available.{context7_cli, augment_mcp_cc, rg_available, jq_available}`
...
### Step 6: 初始 lessons.md

```bash
cat > .ai_state/lessons.md << 'EOF'
# Project Lessons (Athena v9.6.2)
...
## 工具
- context7 (npx ctx7): ✓
- augment MCP (CC): 未配置. 配置: 在 ~/.codex/config.toml [mcp_servers] 加 augment-context-engine
- rg: ✓
- jq: ✓
...
- 若 ~/.agents/skills/_athena/pace/templates/_index.md 不存在: 主 agent fallback 用内置 minimum template (不要去掉 Athena 核心 frontmatter 字段)
