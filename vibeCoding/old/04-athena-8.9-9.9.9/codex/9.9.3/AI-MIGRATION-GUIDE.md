# Athena 9.9.3 — AI 引导安装 / 升级 / 数据迁移指南
...
## 场景一 · 全新安装 9.9.3
1. 定位包: `vibeCoding/claude/9.9.3/.claude` (CC) 与 `vibeCoding/codex/9.9.3/.codex` (CX)。
...
4. 校验: 身份行/env 版本 = 9.9.3; skill 数 = 26; 跑 `python3 vibeCoding/scripts/validate-athena-9.9.3.py` (Python 3.11+)。

## 场景二 · 升级旧版 → 9.9.3 (AI 迁移)
读 `CHANGELOG.md` 9.9.3 段, 逐项应用 (仅当用户未自定义):
- 版本标识 → 9.9.3。
...
- `.ai_state` schema 9.9.3 **向后兼容**, 无破坏性变更。
...
## 校验
`node vibeCoding/scripts/test-athena-claude-9.9.3-runtime.cjs` + `python3 vibeCoding/scripts/test-athena-9.9.3-runtime.py` + `python3 vibeCoding/scripts/validate-athena-9.9.3.py` (后两者需 Python 3.11+)。
