...
description: |
  ...
  v9.6.4-hotfix3: 由 prompts/ 迁为 skill (prompts 已 deprecated 且不可隐式调用); 修 schema 目录 + 平台探测.
effort: medium
---

# /athena-init — 项目初始化 (Codex, v9.6.4-hotfix3)
...
# 注: v9.6.4-hotfix3 起不再探测语义扫描索引工具 (已退场, 用 rg/grep 兜底)
```
...
- `tools_available.{context7_cli, rg_available, jq_available}`
- `platform_features.{cx_spawn_agent, cx_spawn_agents_on_csv, cx_goal_default_on, ag_parallel_subagents, ag_headless_p}`

判定规则:

- `cx_spawn_agent = true` if `CX_VERSION` >= 0.128
- `cx_spawn_agents_on_csv = true` if `CX_VERSION` >= 0.128 且 `[features].multi_agent = true`
