...
## v9.6.4 新增: finishing-a-development-branch (借 Superpowers)
...
### Step 1: spawn polish_worker subagent

```python
Task(
    subagent_type="polish_worker",
    prompt="""
    Polish sprint {slug}. Refactor/System 路径.
    
    检查 5 项: 临时代码 / 注释 / 冗余 / 低效 / 过度设计.
    读 sprints/{slug}/reviews/pass1.md 处理 P1/P2 findings.
    
    完成后:
    1. 写 sprints/{slug}/cleanup-pass.md
    2. 触发 finishing-a-development-branch (跑测试 + 用户选 merge/PR/继续/丢弃)
    3. 触发 compound/ 沉淀 (learning + decision)
    4. (若 ≥5 文件改动) 提示 /architect-doc update
    """,
)
```
...
- `cleanup-pass.md` 存在 (沿用 v9.6.2)
- `architecture/` 已更新 (≥5 文件改动时, 铁律 15)
- `design_changed_after_impl != true` (v9.6.4 新)
