# PACE References · Stages (v9.9.0, Codex)
...
1. Codex `plan_mode_reasoning_effort = "xhigh"` 已在 config.toml 生效;
   `_index.plan_model: fable` 为 CC 端专属 (claude-fable-5 审议, Mythos 级); CX 端等价姿势 = xhigh reasoning effort (已是默认)
...
3. 黄区: spawn_agent `generator` subagent (TDD: 测试先, 代码后); **v9.9.0 (U1)**: Feature+ 的 impl 必须留下 generator 痕迹 (subagent-log.md), delivery-gate 在 ship 机械验 — 主 thread 全程亲手写 = block
