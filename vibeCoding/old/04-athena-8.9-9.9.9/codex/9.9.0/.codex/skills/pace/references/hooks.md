# PACE References · Hooks & Compound (v9.9.0, Codex)
...
| PreToolUse(Bash) | pre-bash-guard.py | 灾难命令拦截 + 铁律[零写入] 红区 spawn_agent --cwd 强制 + worktree 命令跟踪; v9.9.0 加 git push 门禁 (stage != ship 拦, 防 CC 2.1.198 worktree 自动 PR 绕过 review — CC 端行为, CX 端门禁同样生效; 逃生 ATHENA_ALLOW_PUSH=1) |
| PostToolUse(Bash) | subagent-retry.py + evidence-collector.py + index-updater.py | spawn_agent 日志/roadmap 推进 + 过程证据收集; v9.9.0 补挂 index-updater (Goals 长自主 loop 中 re-route 检测不滞后) |
| SubagentStop | subagent-tracker.py | subagent 完成记录 (v9.7.1: 读取官方 agent_type / agent_id, 兼容旧字段) |
| Stop | delivery-gate.py | 交付门禁 (exit 0 + JSON decision); v9.9.0 加 U1 (impl 必须留 generator 记录) / U2 (critic 最少轮数) / design mtime 主检测 (CX 文件写不触发 hook, 无 design-change-detector — ship 时 mtime 重算是本端主检测) |
