...
# /athena-setup — Athena 全局首次配置 (v9.9.0)
...
```bash
# 分发包里的 .claude / .codex 目录 (本仓库即 vibeCoding/claude/9.9.0/.claude 和 codex/9.9.0/.codex).
# 优先环境变量; 否则尝试 cwd 下常见布局; 找不到就报错让用户显式指定 (绝不凭空生成).
CC_PKG="${ATHENA_CC_PKG:-}"
CX_PKG="${ATHENA_CX_PKG:-}"
for c in "$CC_PKG" "$PWD/.claude" "$PWD/cc/.claude" "$PWD/claude/9.9.0/.claude"; do
  [ -n "$c" ] && [ -f "$c/settings.json" ] && CC_PKG="$c" && break
done
for c in "$CX_PKG" "$PWD/.codex" "$PWD/cx/.codex" "$PWD/codex/9.9.0/.codex"; do
...
cp -r "$CC_PKG/skills/"*    ~/.claude/skills/                # 23 目录 (v9.9.0 +athena-vm; 9.8.0 注释的 21 系历史计数错误, 实为 22)
# 自检
...
cp -r "$CX_PKG/skills/"*     ~/.codex/skills/                # 23 目录, config.toml 注册 22 (v9.9.0 +athena-vm)
# 自检
...
> ⚠️ **v9.9.0 · CX hook trust**: Codex hooks GA 后按**内容哈希**记录信任 — 本 setup 每次覆盖 hooks 都会使其
> 全部变为 untrusted 待审. 部署后首次进 codex 会逐个提示信任, 属预期; 自动化/CI 场景用
> `--dangerously-bypass-hook-trust` (名字已经告诉你后果, 自负责).
...
| Skills | `~/.claude/skills/` (23 目录) | `~/.codex/skills/` (23 目录 / 注册 22) |
