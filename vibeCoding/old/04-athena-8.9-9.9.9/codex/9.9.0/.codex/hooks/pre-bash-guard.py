...
职责:
  ...
  4. v9.9.0 新: git push 门禁 — CC 2.1.198 起后台 agent 在 worktree 完成后默认自动 commit+push+开 draft PR,
     绕过 Athena 的 review→ship 门禁顺序 (CC 端行为, CX 端 push 门禁同样生效).
     对策: Athena 项目 (有 .ai_state) 中 stage 非空且 != ship 时 block git push.
...
def main() -> int:
    try:
        ...
        if ai_state:
            ...
            # 4. v9.9.0: git push 门禁 (stage != ship 不许推, 防 worktree 自动 PR 绕过 review)
            if re.search(r"\bgit\s+push\b", command) and "ATHENA_ALLOW_PUSH=1" not in command:
                stage = read_field(ai_state / "_index.md", "stage")
                if stage and stage != "ship":
                    sys.stderr.write(
                        f"[pre-bash-guard] BLOCKED: stage={stage} != ship, git push 必须走完 review 门禁后在 ship stage 执行.\n"
