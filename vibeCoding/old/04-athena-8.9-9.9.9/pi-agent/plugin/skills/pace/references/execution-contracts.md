...
```bash
node ~/.pi/agent/skills/pace/scripts/review-binding.cjs prepare --cwd /absolute/worktree --mode implementation
node ~/.pi/agent/skills/pace/scripts/review-binding.cjs bind --cwd /absolute/worktree --run '<prepare 返回的 run>' --receipt /absolute/dispatch-tool-result.json
node ~/.pi/agent/skills/pace/scripts/review-binding.cjs accept --cwd /absolute/worktree --run '<同一 run>' --receipt /absolute/completion-tool-result.json
```
...
等待期间不重复 prepare。只有现场确认旧原生请求已结束或失效，才运行 `node ~/.pi/agent/skills/pace/scripts/review-binding.cjs supersede --cwd /absolute/worktree --run '<旧 run>'`；该命令不取消原生任务。针对性复核准备新 run 并使用新的独立真实目标，不能复用旧 target 接收晚到结果。
