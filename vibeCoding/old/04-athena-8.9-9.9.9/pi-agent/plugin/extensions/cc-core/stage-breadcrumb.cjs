...
function extractStageSection(stage) {
  const pkgStages = path.join(__dirname, '..', '..', 'skills', 'pace', 'references', 'stages.md');
  ...
  const stagesPath = [pkgStages, piStages, ccStages].find((p) => fs.existsSync(p));
  if (!stagesPath) return null;
