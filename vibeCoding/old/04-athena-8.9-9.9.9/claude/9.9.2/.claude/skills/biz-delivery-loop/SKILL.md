...
5. impl FE：调用 `quantum-codegen (mode=page)` 生成 mock demo，启动 `runtime-env` 探活。
6. CP2：机器门禁确认 demo 可运行；失败回到 FE impl。
7. impl DB：调用 `quantum-codegen (mode=db)` 产出表设计文档和 DDL SQL。
8. CP3：人工 schema 评审；失败回到 DB design/impl。
9. impl BE：调用 `quantum-codegen (mode=module)` 生成后端模块并编译。
10. CP4：机器门禁确认编译、契约和基础质量；失败回到 BE impl。
11. runtime-verify：调用 `quantum-codegen (mode=unit)`、集成契约 diff、`quantum-codegen (mode=e2e)`、`quantum-codegen (mode=security)`。
12. 可选读数：调用 `quantum-data` 读取运行期只读数据；只接受 Capability Manifest 声明的 read 能力。
