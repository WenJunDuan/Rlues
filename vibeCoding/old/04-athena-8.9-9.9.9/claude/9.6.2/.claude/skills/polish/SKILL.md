...
# 3. 加载规则
cat ~/.claude/rules/doc-style.md
cat ~/.claude/rules/coding-standards.md
```
...
模板见 `~/.claude/skills/pace/templates/details/cleanup-pass.md`. 5 段对应 5 个检查项 + 合并 review 意见段.
...
CC 端 polish stage 由**主 agent**执行 (不 spawn subagent).
...
可选: 主 agent 可 spawn Task `subagent_type: reviewer` 做"polish 后的二次 review", 但不强制.

## delivery-gate 强制

`~/.claude/hooks/delivery-gate.cjs` 会检查:
