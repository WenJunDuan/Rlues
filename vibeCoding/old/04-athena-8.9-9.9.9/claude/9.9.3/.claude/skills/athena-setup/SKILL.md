---
name: athena-setup
description: Fresh-install Athena v9.9.3 globally, install a missing Claude Code or Codex endpoint, or verify an existing v9.9.3 installation. Use for first-time global setup; route older installations to athena-migrate instead of overwriting them.
---

# Athena setup v9.9.3

Run the bundled installer. It evaluates Claude Code (CC) and Codex (CX) independently.
The repository distribution pair is `vibeCoding/claude/9.9.3/.claude` and
`vibeCoding/codex/9.9.3/.codex`.

```bash
# From the Rlues repository root: inspect first, then install.
python3 vibeCoding/codex/9.9.3/.codex/skills/athena-setup/scripts/setup-athena.py \
...
all packaged assets exist, hashes match, the version marker is `9.9.3`, and CX skill entries no
longer point at `~/.codex/skills`.

```bash
python3 vibeCoding/codex/9.9.3/.codex/skills/athena-setup/tests/test_setup_991.py
```
