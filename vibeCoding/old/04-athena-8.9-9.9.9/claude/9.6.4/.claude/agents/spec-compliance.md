...
## Stop hook prompt 类型自动触发

`cc/.claude/settings.json` 有 Stop hook prompt 类型:

```json
{
  "type": "prompt",
  "prompt": "Read .ai_state/_index.md. If stage='review' AND latest reviews/pass1.md missing '## Spec Compliance' section, output decision='block' with reason='spawn spec-compliance subagent first'. Else decision='approve'."
}
```

它会自动 block 主 agent 直到你跑过.
