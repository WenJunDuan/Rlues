# PACE References · Stages（Pi）

本页是本发行包阶段义务的唯一正文。其他 agent/skill/模板引用本页；恢复、review 绑定、证据与整合细则见 [execution-contracts.md](execution-contracts.md)。PACE 控制工作，.ai_state 保存合同与事实；Pi-only 可完成适用全流程。Pi 无 CC 对称工具，不伪造。
...
- 绿区：≤3 文件且合计 ≤150 行，或 Hotfix/Quick/Bugfix，主 session 可直接实施。
- 黄区：单模块 Feature，`/generator` 或独立 pi session；worktree 可选。
- 红区：Refactor/System 或 ≥2 并行 writer。主 session 先 `git worktree add`，再在该目录开新 pi session（或 `/generator`）。首条 shell `pwd`。无 `isolation: worktree`。
...
Refactor/System 在 runtime-verify 后、implementation review 前执行。polish_worker 沿用当前实现 worktree 与明确写集；不另开 worktree。`/polish-worker` 或同 worktree 新 session。
...
发起一次独立多维 review：`/reviewer` 或新 pi session 只读；无 `/code-review`。无需 CC/CX/Grok。
...
产出用电宝体=电报体; 声明见 `~/.pi/agent/rules/doc-style.md`「电宝体」。门禁标题/字段名不改。
