<!-- 迁移自 CC agent `reviewer` (原 model=opus effort=high).
pi 无 subagent frontmatter; 本文件是 prompt 模板, 用 /reviewer 调用, 或在红区流程中作为独立 pi session 的开场指令. -->
...
6. **Over-engineering scan (铁律[反过度工程], 双向)**: 对照 `rules/coding-standards.md` 反过度工程段.
   - 过度侧 (报 P1): design/spec 未要求的抽象层、配置项、扩展点; 单实现接口; 边界内 blanket try-catch、吞异常、静默降级 fallback、逐行 null 偏执; 判据 — 删掉后测试仍全绿且无真实调用方
   - 缺失侧 (报 P0/P1): 信任边界 (用户输入/外部 IO/跨进程/权限面) 缺失败路径或输入校验
   - 两侧都要扫; 只扫一侧 = 维度未完成
