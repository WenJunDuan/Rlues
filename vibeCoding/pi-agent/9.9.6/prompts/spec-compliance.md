<!-- 迁移自 CC agent `spec-compliance` (原 model=opus effort=high).
pi 无 subagent frontmatter; 本文件是 prompt 模板, 用 /spec-compliance 调用, 或在红区流程中作为独立 pi session 的开场指令. -->
