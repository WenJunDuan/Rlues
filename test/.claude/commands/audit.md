...
- recommended: `payload.policy_pack.standard_file`（由制度文档生成的标准文件）
...
2. `@policy-researcher`：读取用户提供的制度文档，生成/补全 `policy_pack.standard_file` 与制度证据。
...
4. `@expense-auditor`：汇总步骤 1–3 结果，执行 skill rules 判定并输出最终结论。
   ...
   - skill 仅负责通用判定逻辑，具体制度阈值来自 `payload.policy_pack`。
