...
path: "System"
stage: "design"
author: "cc-main (rev 8 + 135ab562 CONCERNS 四子句落实)"
base_commit: "a385d2e"
---
...
**方案史（7 轮审查，档案 reviews/）**：全文法建模 → 窄核黑名单，黑名单三次被「未知 bash 词法」攻破（算术 `((`/`$[`/下标、`${v:-…}` 展开体字面、`${v:offset}` 算术偏移）。根因恒定：否定式论证要求穷举 bash 文法，穷举必漏。用户裁决改**白名单**；rev 7 复核证实词法核心全对抗通过（前五次 P0 的词法根因被结构性关闭），但暴露**另一类**缺口：掩码语义——`bash <<'EOF'` 词法上确是 heredoc，正文却是喂给解释器的 shell 代码，今日 guard 拦（其 `-c` 递归分支证明该覆盖是有意的，含 push 闸门经 heredoc 绕过例），掩码即放行。rev 8（用户批准）补**消费者正集**：掩码只对已知非 shell 消费者生效，shell 与一切未知包装器（sudo/timeout/env/…不可枚举）默认落「不适用=今日行为」——语义层同样白名单化，fail-open 仍不可构造。
...
- **消费者正集（rev 8）**：首 token 的 basename 必须 ∈ {python3, python, node, tee, cat}，否则不适用。依据：shell 解释器的 stdin 是 shell 代码，guard 今日对其有有意覆盖（`-c` 递归分支同构），掩码=删除该覆盖；而包装器（sudo/env/timeout/nice/…）不可枚举，故默认拒绝而非枚举排除。正集内消费者的 stdin 不是 shell 代码——其唯一「危险」是文本落盘/解释器执行，与 Write 工具写同样内容完全等价（Write 不经 guard），heredoc 对此的今日扫描属偶然覆盖非有意防线。失败方向：未知消费者=今日行为（过拦）。
...
- 正文/终止行按**物理行**：终止行与 NAME 全等（`<<-` 额外允许并剥离前导 tab——仅 tab；允许尾随 `\r`——**此条为有意偏离 bash**（bash 中 `EOF\r` 不闭合），方向=提前结束掩码=过拦无损；尾随空格不构成闭合，与 bash 一致）。未闭合 ⇒ 不适用（不新增拦截）。
- 终止行之后的行 = 命令上下文，照常分析。

**安全论证（两层白名单）**：词法层——文法首行无算术入口/展开体/引号转义注释分隔，bash 对此形状只有一种解析（词序列+heredoc 重定向），rev 7 复核全对抗通过；语义层——掩码只对消费者正集生效，shell 与未知包装器默认不适用。两层的未知形态都落「今日行为」，fail-open 不可构造。
...
| AC | 判据 |
|---|---|
| AC1 | 白名单窄形 quoted 免疫：`evidence/live-samples.md` 四形态最小等价样本（已逐条实测今日 BLOCK）重放放行（先红），`tee` 正向真实路径样本同放行；`<<-` 仅剥 tab、终止行尾随空格不闭合、尾随 `\r` 闭合（正向钉）；**窄形闭合后同命令后续行照常分析**（python3 heredoc 后随 push 行仍 PUSH-GATE / 正文内 push 字样被掩不拦，双钉——复核 P1-1）；**正集成员矩阵**：五成员各一正例 + `perl` 一负例（不适用） |
| AC2 | unquoted 并集：`# $(危险替换)` 正文今日放行、改后拦（先红）；替换先于撇号的正文今日拦、改后仍拦（主流不变钉）；撇号先于替换=今日已放行的既存缺陷不因本切片改变（等价断言） |
| AC3 | 非窄形字节等价矩阵（**全量枚举清单**）：七轮审查全部反例——五算术逃逸、`${v:-<<'a' }` 展开体、`${v:1<<'a' }` 偏移、行续、双引号内行尾反斜杠、跨行引号、注释行伪声明、内嵌声明、多 heredoc、双声明、`<<<`、转义定界 `<<\EOF`、混合引用定界 `<<E"OF"`、引号内 `grep '<<EOF'`、`<<'EOF'x` 残留、非首行声明、未闭合、同行尾随内容、管道/分号/花括号/反引号首行、**解释器族 `bash <<'EOF'` / `sudo bash <<'EOF'` / `bash -s <<'EOF'` / `timeout 5 bash <<'EOF'` / push 绕过例 / `cat <<'EOF' | bash`（正集消费者+正文直通 shell 的最高价值反例）**——逐一断言新旧 `analyze` 输出完全相等（解释器族=今日 BLOCK 保持） |
...
词法层可错方向=文法写宽（TOKEN 字符集混入词法活性字符）——字符集逐字符核过 bash 词法惰性，rev 7/8 两轮对抗通过。语义层可错方向（复核 P2-2）：①正集误收「stdin 即 shell 代码」的消费者——五成员经复核实测均零有意防线，新增成员须重过同一论证；②basename 欺骗（`./cat` 实为 shell 包装）——与 guard 既有全局 `path.basename` 语义一致，非本切片引入，威胁模型内接受并记录。误判窄=维持今日误拦，零安全损失。
