> 注：本文件为 R3/R4 过程稿，已被 `../design.md` 与 `../roadmap.md` 取代，保留作反驳痕迹。
...
> **以 `ai-state-v2.md` 为准**（2026-09-24 修订）：新增 `docs/{requirements,research,reports}`、`decisions/`、唯一问题账 `issues.md`，拆掉 `compound/`，补齐需求→拆解→sprint→归档→延后的完整生命周期，以及超前设计点 X1–X7（CC Tasks 同步、PACE 阶段做成 CC 动态 workflow、需求到证据全链追溯、Grok Build 第四适配端）。下文保留作 R3 痕迹。
