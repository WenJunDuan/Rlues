...
path: "System"
stage: "design"
author: "cc-main (rev 8: 词法核心过审后补掩码语义层——消费者正集; 用户批准)"
base_commit: "a385d2e"
---
...
| AC | 判据 |
|---|---|
| AC1 | 白名单窄形 quoted 免疫：`evidence/live-samples.md` 四形态最小等价样本（已逐条实测今日 BLOCK）重放放行（先红），`tee` 正向真实路径样本同放行；`<<-` 仅剥 tab、终止行尾随空格不闭合、尾随 `\r` 闭合（正向钉） |
| AC2 | unquoted 并集：`# $(危险替换)` 正文今日放行、改后拦（先红）；替换先于撇号的正文今日拦、改后仍拦（主流不变钉）；撇号先于替换=今日已放行的既存缺陷不因本切片改变（等价断言） |
| AC3 | 非窄形字节等价矩阵（**全量枚举清单**）：七轮审查全部反例——五算术逃逸、`${v:-<<'a' }` 展开体、`${v:1<<'a' }` 偏移、行续、双引号内行尾反斜杠、跨行引号、注释行伪声明、内嵌声明、多 heredoc、双声明、`<<<`、转义定界 `<<\EOF`、混合引用定界 `<<E"OF"`、引号内 `grep '<<EOF'`、`<<'EOF'x` 残留、非首行声明、未闭合、同行尾随内容、管道/分号/花括号/反引号首行、**解释器族 `bash <<'EOF'` / `sudo bash <<'EOF'` / `bash -s <<'EOF'` / `timeout 5 bash <<'EOF'` / push 绕过例**——逐一断言新旧 `analyze` 输出完全相等（解释器族=今日 BLOCK 保持） |
