...
path: "System"
stage: "design"
author: "cc-main (rev 2: a3f6ebdb REWORK 全落实)"
base_commit: "a385d2e"
---
...
- **heredoc 识别单一来源**：`_shell-lex` 新增 `heredocSpans(command)`（CX `heredoc_spans`），语义规则写死（review P0/P1 全落实）：
  ...
  - 未闭合（无终止定界符行）⇒ 哨兵 → guard **新增** fail-closed 拦截（基线实测今日不拦；理由：bash 侧即语法错误，无合法用例）。
...
| AC3 | 命令位置 fail-closed：**定界符同行**危险命令照拦（先红判据：掩码不得吞同行）；多 heredoc 声明序用例；heredoc 前后段照拦；未闭合 heredoc **新增**拦截；herestring/算术左移/引号内字面/转义定界四类负向不误拦（转义定界正文免疫=quoted）；既有危险模式全回归 |
...
1. 活体样本重放（含本轮返工脚本再次被拦的第 5 例，先红）；2. quoted/unquoted 矩阵含 # 行首正文先红；3. 同行命令/多 heredoc/未闭合/herestring/算术/引号内/转义定界负向全套；4. lexer 缺失与运行期异常回退各一例（缺失≠放行）；5. 证据侧 heredoc 分段矩阵；6. CX 长命令截断先红；7. 字节钉调整后全套绿。
