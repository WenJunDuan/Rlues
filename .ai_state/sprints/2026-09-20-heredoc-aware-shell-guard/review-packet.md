---
source_design_sha256: "4f8fb21b5b4a2843f69947870f61ef02eac9368c3455256410578df2903e5748"
mode: "design"
---
...
| AC | 判据 |
|---|---|
| AC1 | 四形态最小等价样本（已实测今日 BLOCK）重放放行（先红）+ 真实路径正向样本；`<<-`/终止行三细则正向钉（`\r` 已标有意偏离） |
| AC2 | unquoted 并集：`# $(…)` 洞先红后拦；替换先于撇号今日拦仍拦；撇号先于替换既存放行不变（等价断言） |
| AC3 | 非窄形字节等价全量清单：七轮全部反例 + 解释器/包装器族（bash/sudo bash/bash -s/timeout bash/push 绕过）逐一新旧 analyze 输出相等 |
