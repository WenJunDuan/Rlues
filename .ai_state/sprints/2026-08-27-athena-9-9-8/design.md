...
path: "System"
created: "2026-08-27"
last_updated: "2026-08-27T11:24:44+08:00"
document_status: "awaiting-independent-review"
...
# Design — Athena 9.9.8 Thin PACE Control Plane

> 本稿修订 Grok 草案，但不自评、不写 VERDICT。当前 System sprint 由用户交给独立 Claude，执行同目录的派生材料 `review-packet.md`；这次跨模型复盘不等于把固定设计审查加回所有路径。
...
2. 实现、运行验证、代码清理都结束后，只发起一次多维 code-review 请求；官方 harness 内部是否并行多个 reviewer 不计作 Athena 的多轮流程。
3. 机械事实由 gate/test 直接验证；模型 review 只处理需要判断的 spec 偏差、正确性、安全、测试风险与过度工程。
4. `_index.md` 只保留有界活状态；历史 sprint 冷归档、telemetry 退出 Git、历史目录默认不进入上下文或递归检索。
5. 模型与 effort 按任务风险路由，先用代表性 eval 再改 fresh-install 默认值；迁移绝不覆盖用户已有配置。

目标是把“思考、审查、记账”占用从用户观察到的约 2/3 降到不高于 1/3，同时不降低交付门禁通过率。
...
| 要求 generator 与 polish 都去掉 `skills: [pace]` | 9.9.6 CC 只有 generator 命中；按真实双端文件逐项修，不写虚假对称 AC |
...
request → route → design/packet → implementation → tests/runtime-verify
        → code-changing polish → one native review request → ship/state sync
```

若 review 发现必须修改代码，原结果因 diff hash 变化而失效，只对 open findings 与增量 diff 发起一次目标复核；不预排 pass2/pass3，也不重新运行固定三角色。
...
| 端 | 首选入口 | Athena 提供 |
|---|---|---|
| CC | `/code-review` 或可用的 managed Code Review | 稳定 `REVIEW.md` + sprint packet + diff/evidence |
...
| 黄：可修复的流程/质量信号 | warning + 一条解锁动作 | VM 未覆盖、design 超预算、可选 artifact 缺失、历史索引陈旧、parser 对已证明只读 AST 不确定 |
...
- `Stop` 只校验当前 stage 真正到期的义务；失败返回一个当前角色能完成的动作，同因三次后交还用户，不用 continuation 制造活锁。
- 不把 LLM judge/verifier 放进同步 hook。当前 Codex 官方只执行 command/MCP hook，`prompt`/`agent` handler 会被跳过；语义验证应是显式 review/eval 调用。
...
- `_index.md` 不超过 12 KiB，任何 history/log 列表最多 10 项；更新 hook 在写入时截断，不靠提醒。
...
- 当前已跟踪的 `token-usage.yaml`/`tool-trace.jsonl` 迁移时保留本地副本，再从 Git index 移除；不删除用户数据。

当前仓库约 3.6 MiB、243 个文件、22 个 sprint；字节主要由 telemetry 占据，查找税则来自平铺历史与无界 `_index`。因此先做有界索引与检索协议，不引入 SQLite/embedding；只有实际测量仍慢时再升级。
...
实施前用 9.9.6 的代表性历史任务做同 effort 与低一档对照，至少覆盖 Quick、Bugfix、Feature、System；记录成功率、gate、有效 findings、tokens、耗时。只有质量非劣且控制面 token 明显下降才改 fresh-install 默认。用户安装态设置永不被 migrate 覆盖。
...
- CC/CX reviewer、generator、SessionStart、continuator、index renderer 全量扫描旧话术。
- 仅真实命中的 generator 去掉 pace 预加载；read-only agent 取消手工 binding。
...
- `_index` 模板/更新 hook 加 12 KiB 与列表上限。
- archive 默认排除，生成可重建 runtime catalog；telemetry 迁到 ignored runtime 目录并设 retention。
...
- **AC3**：实现完成后的默认 Athena 调度只有一次 review 请求和一份 result；live emitter 不再调度 spec-compliance/evaluator/固定 pass2/pass3。
- **AC4**：原生 harness 内部可多 agent；Athena 不重复编排。代码变更使旧 review 的 exact diff hash 失效，只做目标复核。
- **AC5**：R/S 的最终 review 位于所有会改代码的 polish 之后；review 后若代码变化，ship gate 必须 block。
- **AC6**：review result 用结构化 frontmatter；gate 不再依赖 `Critic Findings`、`## Spec Compliance` 或 passN 标题数量。
- **AC7**：Quick/Hotfix/Bugfix/Feature/R/S 的仪式与表一致；hook 只对红区 fail-closed，黄区不阻断、绿区不续跑；测试、安全、权限和 worktree 真边界不降级。
- **AC8**：双端只对齐语义；同事件最多一个同步 blocker；read-only role 无手工 assignment，writer 绑定仍可核验；无害 `rg` quote/regex fixture 通过，旧 2+1 话术不出现在 live emitter。
- **AC9**：`_index` ≤12 KiB 且列表 ≤10；archive 不被默认扫描；runtime catalog 删除后可重建；telemetry 不再由 Git 跟踪。
- **AC10**：canonical 9.9.6 基线、用户安装态与 9.9.8 target 分开记录；migration 保留用户 model/effort，默认降档必须通过代表性 eval。
- **AC11**：双端 validator 与 runtime fixture 0 FAIL；对比基线时，交付成功率/安全门禁不降低，median 控制面 tokens 至少下降 40%，非代码/工具控制面占比不高于 1/3。
- **AC12**：不新增第二状态树、不合并 26 skill、不新增人工维护 catalog；派生 packet 最多 80 行且不复制 design 散文。

## 发布边界

三个切片各自可回滚，review-contract 先落；模型默认和 archive 迁移不与审查门禁绑成一次不可回滚改动。实施对象若含 repo 外安装态，按 harness 外部目标逐文件备份；本设计通过独立 Claude 复盘前 `implementation_authorized` 保持 false。
